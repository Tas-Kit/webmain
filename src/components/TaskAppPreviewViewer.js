...
const TaskAppPreviewViewer = (props) => {
  ...
  return (
    <div>
      ...
      <p><FormattedMessage id="expectedFieldName" />: {task.expected_effor_unit ? `${task.expected_effort_num} ${TIME_UNITS_MAP_TWO[task.expected_effor_unit]}` : <FormattedMessage id="noneText" />}</p>
    </div>
  );
};
